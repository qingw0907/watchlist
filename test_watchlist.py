...
class WatchlistTestCase(unittest.TestCase):
	...
	def test_settings(self):
		...
		response = self.client.post('/settings', data=dict(name='qingw',), follow_redirects=True)
		data = response.get_data(as_text=True)
		self.assertIn('Settings updated', data)
		self.assertIn('qingw', data)
	...
	def test_admin_commnad(self):
		db.drop_all()
		db.create_all()
		result = self.runner.invoke(args=['admin', '--username', 'qingw', '--password', '123'])
		self.assertIn('Creating user...', result.output)
		self.assertIn('Done', result.output)
		self.assertEqual(User.query.count(), 1)
		self.assertEqual(User.query.first().username, 'qingw')
